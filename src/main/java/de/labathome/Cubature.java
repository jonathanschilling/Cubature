 ...
 *
 * A cubature rule takes a function and a hypercube and evaluates the function
 * at a small number of points, returning an estimate of the integral as well as
 * an estimate of the error, and also a suggested dimension of the hypercube to
 * subdivide.
 *
 ...
 *
 * 2) Pick the hypercube with the largest estimated error, and divide it in two
 * along the suggested dimension.
 *
 * 3) Goto (1).
 *
 ...
 * (4), 437-451 (1991).
 *
 * Note, however, that we do not use any of code from the above authors (in part
 * because their code is Fortran 77, but mostly because it is under the
 * restrictive ACM copyright license). I did make use of some GPL code from
 * Rudolf Schuerer's HIntLib and from the GNU Scientific Library as listed in
 * the copyright notice above, on the other hand.
 *
 ...
 * evaluating multiple points in a single call (as opposed to multiple functions
 * in a single call). (Although Dmitry implemented a working version, I ended up
 * re-implementing this feature from scratch as part of a larger code-cleanup,
 * and in order to have a single code path for the vectorized and non-vectorized
 * APIs. I subsequently implemented the algorithm by Gladwell to extract even
 * more parallelism by evalutating many hypercubes at once.)
 *
 * TODO:
 *
 * For high-dimensional integrals, it would be nice to implement a sparse-grid
 * cubature scheme using Clenshaw-Curtis quadrature. Currently, for more than 7
 * dimensions or so, quasi Monte Carlo methods win.
 *
 * Berntsen et. al also describe a "two-level" error estimation scheme that they
 * claim makes the algorithm more robust. It might be nice to implement this, at
 * least as an option (although I seem to remember trying it once and it made
 * the number of evaluations substantially worse for my test integrands).
 *
 */
public class Cubature {

    /**
     * Integrate the given function {@code integrand} in the range [xmin:xmax]
     * to a relative tolerance {@code relTol} or absolute tolerance  {@code absTol} or until {@code maxEval}
     * function evaluations were used.
     *
     * @param integrand function to integrate; must map t[xdim][nEvalPoints] ->
     *                  f[fdim][nEvalPoints]
    ...
    public static double[][] integrate(
        ...
        if (xmax == null || xmax.length != dim) {
            throw new RuntimeException(
                    "xmin and xmax must have the same length but have " + xmin.length + " and " + xmax.length);
        }

        // determine dimensionality of integrand by evaluating it once in the center of
        // the given integration interval
        double[][] center = new double[dim][1];
        for (int i = 0; i < dim; ++i) {
            center[i][0] = (xmin[i] + xmax[i]) / 2.0;
        }

        double[][] f = integrand.apply(center);
