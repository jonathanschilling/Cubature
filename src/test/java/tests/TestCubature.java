...
public class TestCubature {
    ...
    void testThreeDimensionalGaussian() {
        ...
        double[][] val_err = Cubature.integrate(gaussianNd,
                xmin, xmax,
                relTol, 0.0, CubatureError.INDIVIDUAL,
                0);
    ...
    void testF0() {
        ...
        UnaryOperator<double[][]> f0 = (double[][] x) -> {
        ...
        double[][] F = Cubature.integrate(f0,
                new double[] { 0.0, 0.0 }, // lower integration limit
                new double[] { 1.0, 1.0 }, // upper integration limit
                tol, // relative tolerance
                0.0, // no absolute tolerance requirement
    ...
    void testF1() {
        ...
        UnaryOperator<double[][]> f1 = (double[][] x) -> {
            double[] ret = new double[x[0].length];
            for (int i=0; i<x[0].length; ++i) {
                double dx = x[0][i] - 0.5;
                double sum = dx * dx;
        ...
        double[][] F = Cubature.integrate(f1,
                new double[] { 0.0, 0.0 }, // lower integration limit
                new double[] { 1.0, 1.0 }, // upper integration limit
                tol, // relative tolerance
                0.0, // no absolute tolerance requirement
